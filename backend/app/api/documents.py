...
def reindex_document(document_id: int, db: Database, _user: AdminUser) -> dict[str, object]:
    document = db.get(Document, document_id)
    if document is None:
        raise HTTPException(status_code=404, detail="文档不存在")
    if document.status in {DocumentStatus.QUEUED, DocumentStatus.PROCESSING}:
...
def delete_document(document_id: int, db: Database, _user: AdminUser) -> dict[str, object]:
    document = db.get(Document, document_id)
    if document is None:
        raise HTTPException(status_code=404, detail="文档不存在")
